...
public class MovementServiceImpl implements MovementService {
    ...
    private Mono<MovementDto> registerMovementOfAccounts(MovementDto movementDto){
        return Mono.justOrEmpty(movementDto)
                ...
                .flatMap(cus -> productService.getProductByCustomerAndId(cus.getId(), movementDto.getProduct().getId()))
                ...
                .flatMap(product -> {
                    ...
                    if(product.getId() == null || product.getId().isEmpty() || product.getId().equals("") || customer.getId() == null || customer.getId().isEmpty() || customer.getId().equals("")) {
                        ...
                        Date firstOfMonth = Date.from(today.withDayOfMonth(1).atStartOfDay().atZone(ZoneId.systemDefault()).toInstant());
                        Date endOfMonth = Date.from(today.withDayOfMonth(today.lengthOfMonth()).atStartOfDay().atZone(ZoneId.systemDefault()).toInstant());
                        return movementRepository.countByProductIdAndDateBetween(product.getId(), firstOfMonth, endOfMonth)
                                .flatMap(count -> {
                                    if(product.getMax_movement_limit() == null){
                                        return Mono.empty();
                                    }else if(count < product.getMax_movement_limit()){
                                        return this.insertMovementDefault(movementDto);
                                    }else{
                                        return Mono.empty();
                                    }
                        ...
                        LocalDate today = LocalDate.now();
                        if(product.getSingle_day_movement().equals(today.getMonthValue())) {
                            Date startOfDay = Date.from(LocalDateTime.of(today, LocalTime.MIN).atZone(ZoneId.systemDefault()).toInstant());
                            Date endOfDay = Date.from(LocalDateTime.of(today, LocalTime.MIN).atZone(ZoneId.systemDefault()).toInstant());
                            return movementRepository.countByProductIdAndDateBetween(product.getId(), startOfDay, endOfDay)
                                    .flatMap(count -> {
                                        if (count == 0) {
                                            return this.insertMovementDefault(movementDto);
                                        } else {
    ...
    private Mono<MovementDto> registerMovementOfCredits(MovementDto movementDto) {
        return Mono.justOrEmpty(movementDto)
                ...
                .flatMap(product -> {
                    ...
                    if (product.getId() == null || product.getId().isEmpty() || product.getId().equals("") || customer.getId() == null || customer.getId().isEmpty() || customer.getId().equals("")) {
                        return Mono.empty();
                    }else if(movementDto.getType().equals(MovementMongo.MOVEMENT_TYPE_2) && product.getType().equals(Product.PRODUCT_TYPE_5)){
                        return Mono.empty();
                    }else if(movementDto.getType().equals(MovementMongo.MOVEMENT_TYPE_2) && product.getType().equals(Product.PRODUCT_TYPE_4) && movementDto.getAmount()>product.getCredit_limit()){
                        return Mono.empty();
                    }else{
                        return this.insertMovementDefault(movementDto);
                    }
